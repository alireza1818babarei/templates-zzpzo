...
if (siteHeader && siteMenuToggle && siteHeaderNav) {
   const setSiteMenuOpen = (isOpen) => {
      siteHeader.classList.toggle('is-menu-open', isOpen);
      siteMenuToggle.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
      siteMenuToggle.setAttribute('aria-label', isOpen ? 'Menu sluiten' : 'Menu openen');
   };

   siteMenuToggle.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      setSiteMenuOpen(!siteHeader.classList.contains('is-menu-open'));
   });

   siteHeaderNav.querySelectorAll('a').forEach(link => {
   ...
   window.addEventListener('resize', () => {
      if (window.innerWidth > 900) setSiteMenuOpen(false);
   });
}
